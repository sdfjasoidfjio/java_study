import java.util.*;
 ...
 class SamsungTV implements TV{ // 문제: 위 계층구조 그림을 참조하여 상속 및 구현 등의 적절한 조치를 취하라.
    ...
    public void volumeUp(){
        System.out.println("SamsungTV: voume changeed from " + volume + " to " + ++volume);
    }
 ...
 class LGTV implements TV{ // 문제: 위 계층구조 그림을 참조하여 상속 및 구현 등의 적절한 조치를 취하라.
    ...
    public void display(String message){
        System.out.println("   LGTV: " + message);
    }
 }
 //-----------------------------------------------------------------------------
// SetTopBox 클래스는 수정하지 마시오.
 abstract class SetTopBox {
    ...
    SetTopBox(String ipAddress) { this.ipAddress = ipAddress; }
    void setTV(TV tv) { this.tv = tv; } // SetTopBox가 제어할 TV를 결합한다
    ...
    void run(Scanner scan) {
        while(true) {
            ...
            if      (input == 0)    break;
            else if (input == 1)    tv.volumeUp();
            else if (input == 2)    OTTservice();
        }
    }
 }
...
class SKSetTopBox extends SetTopBox{ // 문제: 위 계층구조 그림을 참조하여 상속 또는 구현 등의 적절한 조치를 취하라.
    ...
    // 문제: 생성자 및 필요한 멤버 함수들 구현하라.
    //      생성자의 매개변수는 아래 main() 함수 내의 객체 생성을 참조하라.
    public SKSetTopBox(String brandName, String ipAddress) {
        super(ipAddress);
    ...
    String getOTTbanner() {
        return "This is " + brandName + ". Welcome!!";
    }
 ...
 class KTSetTopBox extends SetTopBox{ // 문제: 위 계층구조 그림을 참조하여 상속 또는 구현 등의 적절한 조치를 취하라.
    ...
    // 문제: 생성자 및 필요한 멤버 함수들 구현하라.
    //      생성자의 매개변수는 아래 main() 함수 내의 객체 생성을 참조하라.
    public KTSetTopBox(String brandName, String ipAddress) {
        super(ipAddress);
        this.brandName = brandName;
    }
    String getBrandName(){
